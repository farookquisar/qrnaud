...
function playAud()
{
  ...
  // Increment the repeat variable each time the audio is played
  repeat++;

  // Repeat the audio until the repeat variable is 50
  if (repeat < 50)
  {
    audios[playTrck].addEventListener("ended", playAud);
  }
...
{
  if (isPlaying())
  {//Make Play button active when already not playing
    document.getElementById("pauseBtn").disabled = false;
    document.getElementById("playBtn").disabled = true;
  } else
  {//Make Play button active when already not Puased
